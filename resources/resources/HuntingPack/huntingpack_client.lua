    ...
    while true 
        ...
        if ourTeamType == 'driver' then
            ...
            if lastVehicle == 'Firetruk' then
                SetVehicleCheatPowerIncrease(ourDriverVehicle, 0.70)
                ...
                if timeBelowSpeed > maxTimeBelowSpeed then
                    ...
                    if shouldNotifyAboutDeath then
                        if defenderName ~= '' then
                            TriggerServerEvent('OnNotifyBlownUp', 'Driver: ' .. GetPlayerName(PlayerId()) .. ' || Defender: ' .. defenderName, totalLife)
                        else
            ...
            if ourTeamType == 'driver' then
                ...
                if 15 - (GetGameTimer() - startTime)/1000 > 0 and totalLife < 15 then
                    ...
                    if defenderName == GetPlayerName(PlayerId()) then
                        AddTextComponentString(("Defend the Truck!\n%.1f"):format(15 - (GetGameTimer() - startTime)/1000)) 
                    else
...
RegisterCommand('respawn', function(source, args)
    if totalLife > 0 and ourTeamType ~= 'driver' and ourTeamType ~= 'defender' then
        print('Requesting Start for '.. GetPlayerName(PlayerId()) .. ' in progress')
        TriggerServerEvent('OnRequestJoinInProgress', GetPlayerServerId(PlayerId()))
    end
end, false)
...
Citizen.CreateThread(function()
    while true do
        ...
        if not gameStarted or (GetGameTimer() - startTime)/1000 < 15 then 
            DrawRect(0.75, 0.265, 0.5, 0.07, 252, 10, 12, 200)
            DrawRect(0.75, 0.525, 0.5, 0.45, 42, 42, 43, 150)
            SetTextFont(1)
            SetTextProportional(0)
            SetTextScale(0.0, 1.0)
